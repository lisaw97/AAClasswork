...
def population_constraint
  ...
  execute(<<-SQL)
    ...
    from countries
    where population >
    (select population from countries where name = 'Canada') and
    population <
...
def sparse_continents
  ...
  from countries
  join (select continent from countries group by continent having max(population) < 25000000) as sparse_countries
    on countries.continent = sparse_countries.continent;
  SQL
end
